...
let
  ...
  matchingMergers = xs: builtins.all (x: isMerger x) xs && builtins.all (x: isTerminal x.item) xs;
  matchingTerminals = builtins.all isTerminal;

  applyMerge = self: next:
    if isTerminal next && matchingTerminals [ self.item next ] then
      if isIdentity next then
        self.item
      else
        self.mergeOp self.item next
    else
      if matchingMergers [ self next ] then
        ...
        abort "${lib.debug.traceVal next} is not mergeable with ${lib.debug.traceVal self}";
...
in
{
  inherit isMerger hasMergeOp matchingMergers matchingTerminals isIdentity applyMerge;
}
